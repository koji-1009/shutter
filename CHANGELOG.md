...
* `shutter shot --widget '<expression>'` renders one widget without a preview file (`--import` for the files it needs, repeatable; `--size` for its logical size), writing nothing to the project outside `.shutter/`.
...
* google_fonts: files are downloaded once into `.shutter/fonts/google_fonts/`, verified, and served as bundled assets. A font that cannot be obtained makes the shot an error.
