...
* For a state a gesture or typing gives, add `--tap`, `--enter`, `--press`, `--hover`, or `--focus` to both shots, and `--capture screen` for what opens above the widget; `shutter agent` has the details.
